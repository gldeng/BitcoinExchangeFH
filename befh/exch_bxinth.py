...
class ExchGwBxinthRestfulApi(RESTfulApiSocket):
    """
    Exchange gateway Bxinth RESTfulApi
    """
    def __init__(self):
    ...
    def get_order_book_link(cls, instmt):
        return "https://bx.in.th/api/orderbook/?pairing=%s" % instmt.get_instmt_code()

    @classmethod
    def get_trades_link(cls, instmt):
        return "https://bx.in.th/api/trade/?pairing=%s" % instmt.get_instmt_code()
        ...
        if cls.get_trades_timestamp_field_name() in keys and \
           ...
           cls.get_trade_volume_field_name() in keys:
            ...
            date_time = parser.parse(
                raw[cls.get_trades_timestamp_field_name()] + ' +0700'
            ).astimezone(pytz.utc)
...
class ExchGwBxinth(ExchangeGateway):
    """
    Exchange gateway Bxinth
    """
    def __init__(self, db_clients):
        ...
        """
        ExchangeGateway.__init__(self, ExchGwBxinthRestfulApi(), db_clients)
...
if __name__ == '__main__':
    Logger.init_log()
    exchange_name = 'Bxinth'
    instmt_name = 'ETHBTC'
    instmt_code = '20'
    instmt = Instrument(exchange_name, instmt_name, instmt_code)    
    db_client = SqlClientTemplate()
    exch = ExchGwBxinth([db_client])
    instmt.set_l2_depth(L2Depth(5))
    instmt.set_prev_l2_depth(L2Depth(5))
    instmt.set_recovered(False)    
